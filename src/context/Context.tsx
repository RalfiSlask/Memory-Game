...
export const ContextProvider: React.FC<ContextType> = ({ children }) => {
  const [startMenuSettings, setStartMenuSettings] = useLocalStorage<
    StartMenuSettingsType[]
  >("menuSettings", menuSettingsData);
  const [selectedSettings, setSelectedSettings] =
    useLocalStorage<SelectedSettingsType>("selectedSettings", {
      theme: "Numbers",
      playerNumbers: 1,
      grid: "4x4",
    });
  const [playersList, setPlayersList] = useLocalStorage<PlayersType[]>(
    "players",
    []
  );
  const [memoryPiecesList, setMemoryPiecesList] = useLocalStorage<
    MemoryPieceType[]
  >("memorySettings", []);
  ...
  const handleClickOnPiece = (id: number) => {
    ...
    const numberOfClickedPieces = memoryPiecesList.filter(
      (piece) => piece.isClicked
    ).length;
    const updatedMemoryArray = memoryPiecesList.map((piece, index) => {
      if (index !== id || numberOfClickedPieces > 1 || piece.taken)
        return piece;
  ...
  const handleClickOnStartMenuButtons = (
    buttonLabel: string,
    panelId: number
  ) => {
  ...
  const updatePlayersTurn = useCallback(
    (playersList: PlayersType[]) => {
      const updatedPlayersList = [...playersList];
      const PlayerIndex = updatedPlayersList.findIndex(
  ...
  const updatePlayersScore = useCallback(
    (playersList: PlayersType[]) => {
      const updatedPlayersList = [...playersList];
  ...
  useEffect(() => {
    ...
    if (areTwoPiecesClicked(memoryPiecesList)) {
      ...
      if (
        clickedPiecesArray.every((piece) => piece === clickedPiecesArray[0])
      ) {
  ...
  useEffect(() => {
    ...
    if (everyPieceTaken) {
      ...
      const mostPairs = Math.max.apply(
        null,
        playersList.map((player) => player.score)
      );
      const updatedPlayersList = [...playersList];
      updatedPlayersList.forEach((player, index) => {
  ...
  useEffect(() => {
    if (selectedSettings.playerNumbers === 1) {
      ...
      const playerArray = Array.from(
        ...
        (item, index) =>
          (item = { player: index + 1, score: 0, selected: false })
      );
  ...
  useEffect(() => {
    ...
    if (selectedSettings.theme === "Icons") {
      ...
      const finalArray =
        selectedSettings.grid === "4x4"
          ? createFourByFourArray(emptyArray, ICON_ARRAY)
          : createSixbySixArray(ICON_ARRAY);
